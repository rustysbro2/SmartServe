    ...
    try {
      ...
      if (strikeResult) {
        await interaction.reply(
          `User <@${target.id}> has been striked for reason: ${reason}`,
        );
        ...
        if (strikeChannel && strikeChannel.type === 0) {
          ...
          if (strikeMessageId) {
            const strikeMessage = await strikeChannel.messages.fetch(
              strikeMessageId,
            );
            if (strikeMessage) {
            ...
            const sentMessage = await strikeChannel.send({
              embeds: [strikeEmbed],
            });
      ...
      interaction.followUp(
        "An error occurred while processing the strike command.",
      );
    }
  },
};
