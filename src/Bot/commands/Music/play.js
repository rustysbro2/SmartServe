    ...
    try {
      ...
      if (
        ...
        await interaction.reply(
          "I need the 'Connect' and 'Speak' permissions in a voice channel to use this command."
        )
        return
      }
      ...
      if (wasEmpty && musicPlayer.queue.length === 1) {
        console.log(
          'Waiting for AudioPlayer to transition to "Playing" state...'
        )
        await entersState(
      ...
      await interaction.editReply({
        content: `There was an error while executing this command! Error details: ${error.message}`,
        ephemeral: true
      })
    }
  },
