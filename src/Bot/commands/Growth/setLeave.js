  ...
  try {
    ...
    if (rows.length > 0) {
      await pool
        .promise()
        .query(
          "UPDATE guilds SET leave_message_channel = ? WHERE target_guild_id = ?",
          [channelId, guildId],
        );
    } else {
      await pool
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("setleave")
    .setDescription(
      "Set the channel for the bot to send a leave message when removed from a guild",
    )
  ...
  async execute(interaction) {
    ...
    try {
      ...
      interaction.reply(
        `Leave message channel set to ${channel} for all guilds.`,
      );
    } catch (error) {
      console.error("Error setting leave message channel:", error);
      interaction.reply(
        "Failed to set the leave message channel. Please try again.",
      );
    }
  },
