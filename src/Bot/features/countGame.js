...
async function setCountingChannel(guildId, channelId) {
  ...
  try {
    ...
    printDebugMessage('Counting channel ID updated in countingChannels object:', countingChannels[guildId].countingChannelId);
  ...
  try {
    ...
    if (results) {
      results.forEach((result) => {
        ...
        printDebugMessage('Counting channel loaded:', countingChannels[guildId].countingChannelId);
      });
    }
  } catch (error) {
...
async function handleCountingMessage(message) {
  ...
  if (Math.abs(result - expected) < tolerance) {
    ...
    printDebugMessage('Difference:', expected.toString().split('').map((char, i) => expression[i] === char ? ' ' : '^').join(''));
...
async function handleCorrectCounting(message, expression, author) {
  ...
  if (author.id === countingChannel.lastUserCount) {
    ...
    printDebugMessage('Expected next count:', countingChannel.currentCount + countingChannel.incrementValue);
...
async function handleGameFailure(guild, author, reason, message, expectedNumber, increment) {
  ...
  if (oldChannel) {
    ...
    printDebugMessage('Counting channel deleted:', countingChannel.countingChannelId);
  } else {
...
async function updateHighScore(guildId) {
  ...
  try {
    ...
    if (!currentHighScore || countingChannel.totalNumbersCounted > currentHighScore) {
      ...
      printDebugMessage('High score updated:', countingChannel.totalNumbersCounted);
    }
  } catch (error) {
...
async function sendFailureMessage(guild, author, reason, message, expectedNumber, increment) {
  ...
  try {
    ...
    if (oldChannel) {
      ...
      printDebugMessage('Counting channel updated:', countingChannel.countingChannelId);
      ...
      if (updatedChannel) {
        ...
        printDebugMessage('Current increment value:', countingChannel.incrementValue);
        ...
        await updateCountingChannel(guildId);
        printDebugMessage('Counting channel updated in database:', countingChannel.countingChannelId);
      } else {
