...
async function setCountingChannel(channelId) {
  ...
  try {
    await pool.promise().query('INSERT INTO counting_channels (channel_id) VALUES (?) ON DUPLICATE KEY UPDATE channel_id = ?', [countingChannelId, countingChannelId]);
  } catch (error) {
  ...
  try {
    const [rows] = await pool.promise().query('SELECT count FROM counting_channels WHERE channel_id = ?', [countingChannelId]);
    if (rows.length > 0) {
      count = rows[0].count;
    } else {
      // If no count is found in the database, insert a new row with the initial count
      await pool.promise().query('INSERT INTO counting_channels (channel_id, count) VALUES (?, ?)', [countingChannelId, count]);
    }
  } catch (error) {
...
async function incrementCount() {
  ...
  try {
    await pool.promise().query('UPDATE counting_channels SET count = ? WHERE channel_id = ?', [count, countingChannelId]);
  } catch (error) {
...
async function checkCountingChannel(message) {
  ...
  if (channel.id === countingChannelId && !author.bot) {
    ...
    if (inputNumber === count + 1) {
      ...
      await channel.send(`Oops! That number was incorrect. The next number should be ${count + 1}.`);
    }
  }
}
