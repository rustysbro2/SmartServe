...
async function getStrikeEmbed (guildId, client) {
  ...
  const fields = []
  for (const userId in usersStrikes) {
    const userStrikes = usersStrikes[userId]
    const userStrikeReasons = [...new Set(userStrikes)]
    const userStrikeCount = userStrikes.length
