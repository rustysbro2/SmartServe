  ...
  const fetchData = async () => {
    try {
      const response = await limiter.schedule(() =>
        axios.get(`https://top.gg/api/bots/${botId}/check`, {
          params: {
...
async function checkAllGuildMembers(client) {
  console.log('Adding all guild members to the database...');
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    for (const member of members.values()) {
      ...
      const [userDataRows] = await connection.query('SELECT * FROM users WHERE user_id = ?', [member.user.id]);

      if (userDataRows.length === 0) {
        // User doesn't exist in the database, add them
        const userData = {
          user_id: member.user.id,
          voted: 0,
          last_vote_time: null,
