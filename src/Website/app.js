    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      try {
        let user = await pool.query(
          "SELECT * FROM web_users WHERE discordId = ?",
          [id],
        );
        ...
        if (user.length === 0) {
          ...
          const result = await pool.query(
            "INSERT INTO web_users (username, email, avatar, accessToken, discordId, guilds) VALUES (?, ?, ?, ?, ?, ?)",
            [
              username,
              encryptedEmail,
              avatar,
          ...
          const encryptedEmail = encryptEmail(email);
          await pool.query(
            "UPDATE web_users SET username = ?, email = ?, avatar = ?, accessToken = ?, guilds = ? WHERE discordId = ?",
            [
              username,
              encryptedEmail,
              avatar,
